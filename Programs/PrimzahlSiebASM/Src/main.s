...
//short [] primzahlen = new short [170];  // array mit bytes, 
//int k = 0 ; 
//for (int p = 2; p <= n; p = p + 1) {
	if (sieb[p]) == 0) {
		primzahlen[k] = (short) p; //speichert den Indize p des Sieb-Feldes in ein Primzahlen-Feld
		k = k +1;  //k ist am Ende die Anzahl d. Primzahlen
	}
}